...
- 입력 문자열을 리스트로 변환 이후 튜플을 길이순서대로 정렬
- 길이가 작은 튜플에 있는 숫자부터 넣어서 answer 반환
- 정규식을 사용하면 간단하게 풀이가 가능한 것 같다.

## 📌09.08

#### [크레인 인형뽑기 게임](./2019_KAKAO/크레인_인형뽑기_게임.py)

- stack사용

## 📌09.09

#### [파일명 정렬](./2018_KAKAO/파일명정렬.py)

- isdigit으로 숫자 확인
- temp에 head 담고 숫자가 아닌게 있으면 number, tail 저장
- number 뒤에 아무것도 없을 경우 `int(x[1])` 으로 sort가 안되서 에러가 발생해서 for문 이후에 number가 남았을 경우에 따로 처리해줬다.

## 📌09.10

#### [방금그곡](./2018_KAKAO/방금그곡.py)

- C#, D#, F#, G#, A#를 replace를 이용하여 각각 한 글자로 치환
- 이후, 음의 길이를 늘린 다음 재생시간만큼 잘라서 비교
- 재생시간 순서대로 정렬 이후 첫번째 값 출력

## 📌09.13

#### [복서 정렬하기](./위클리_챌린지/복서_정렬하기.py)

- index번호, 승률, 무게, 무거운 상대 이긴 횟수 저장 후 정렬

#### [입실 퇴실](./위클리_챌린지/입실퇴실.py)

- 입실한 사람을 stack에 저장
- 입실, 퇴실시에 stack에 들어있는 인원들을 set에 추가한 후 최종적인 set의 길이를 출력

## 📌09.28

#### [최소직사각형](./위클리_챌린지/최소직사각형.py)

- min, max별로 temp 리스트 만든 후 2번 정렬

## 📌10.05

#### [전력망을 둘로 나누기](./위클리_챌린지/전력망을_둘로_나누기.py)

- i번째 전선을 끊을때마다 adj를 만들어서 bfs로 몇개 방문하는지 체크

## 📌10.08

#### [합승 택시 요금](./2021_KAKAO/합승택시요금.py)

>Dijkstra

- 시작 지점, A, B에서 다익스트라를 이용해서 최단거리 리스트를 구한다.
- 1부터 n+1까지 전체를 돌면서 `answer = min(answer, s_dist[i] + a_dist[i] + b_dist[i])` 로 갱신 후 출력
  -  i번째 지점부터 a,b,s까지의 최단거리

## 📌10.15

#### [체육복](./체육복.cpp)

- 정렬 후 앞, 뒤로 빌려줄 수 있는지 체크
- reserve와 lost에 모두 존재하는 경우엔 빌려주지 못하므로 먼저 예외처리 해줘야 한다.
...
- 노드 1부터 BFS 돌면서 visited에 길이 체크하면서 최댓값 저장
- `visited[i]` = 최댓값인 개수 출력

## 📌10.16

#### [교점에 별 만들기](./위클리_챌린지/교점에별만들기.py)

- `min_x, min_y, max_x, max_y` 과 별의 좌표 저장
- python의 y좌표와 좌표계의 y좌표가 다르므로 max_y부터 min_y 순서대로 넣어야 한다.

## 📌10.20

#### [완주하지못한선수](./완주하지못한선수.py)
...
- Counter 사용해서 현재값과 이동값 사이에 1 차이가 나는 단어로 DFS 진행
